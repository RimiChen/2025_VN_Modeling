...
for _, row in df.iterrows():
    ...
    G.add_node(plot_1, type="event", label=plot_1)
    G.add_node(plot_2, type="event_segment", label=plot_2)
    G.add_node(panel_id, type="panel", label=panel_id)

    # Structural hierarchy edges
    G.add_edge(plot_1, plot_0, relation="subevent_of")
    G.add_edge(plot_2, plot_1, relation="subevent_of")
    G.add_edge(panel_id, plot_2, relation="instantiates")

# === ADD STORYTIME TEMPORAL RELATIONS ===
# Manually define narrative order between plot_1 events
event_story_order = [
    ("Think of family", "Message from family"),
    ("Intro_1", "Get new rice_cooker")
]

for src, tgt in event_story_order:
...
for rel_type, color, style in [
    ...
    nx.draw_networkx_edges(G, pos, edgelist=edges, edge_color=color,
                           ...
                           connectionstyle='arc3,rad=0.2', width=1, style=style)
    nx.draw_networkx_edge_labels(G, pos,
                                 edge_labels={k: v for k, v in edge_labels.items() if k in edges},
                                 font_color='red', label_pos=0.6)

plt.title("Hierarchical Event KG (Top-Down + Narrative Flow)", fontsize=14)
plt.axis("off")
plt.tight_layout()
