...
def build_panel_graph(panel, panel_id, metadata_row):
    ...
    # === ACTIONS (updated: add panel_visual edge)
    for a_idx, action in enumerate(panel.get("actions", [])):
        ...
        if len(parts) >= 3:
            ...
            G.add_edge(panel_visual, action_node, relation="has_action")  # ← NEW

    # === CHARACTERS (updated: add panel_visual edge)
    for char in panel.get("characters", []):
        if char:
            ...
            G.add_edge(panel_visual, char, relation="has_character")  # ← NEW

    # === DIALOGUES (updated: add panel_textual edge)
    for j, dialogue in enumerate(panel.get("textual", {}).get("dialogues", [])):
        ...
        G.add_edge(panel_textual, d_node, relation="has_dialogue")  # ← NEW
        if dialogue:
